# ==============================================================================
# ARQUITECTURA FASE 11: EL DIARIO DEL CAPITÁN (MEMORIA DE SISTEMA)
# Objetivo: Registrar las predicciones para auditoría futura y evitar la amnesia.
# ==============================================================================
...
st.title("💾 Radar IA: Memoria Institucional")
st.markdown("Sistema de análisis multi-activo con registro de predicciones históricas.")

# INICIALIZACIÓN DE LA MEMORIA (Session State)
# Esto crea un 'diario' en la memoria del navegador que no se borra al pulsar botones
...
boton_analizar = st.sidebar.button("Activar Radar e Inmortalizar Predicción")
...
if boton_analizar:
        ...
        try:
            ...
            if not df_raw.empty:
                ...
                resultado = {
                    ...
                    "Precisión Modelo (%)": round(prec, 2)
                }
                resultados_actuales.append(resultado)
                # GUARDAMOS EN EL DIARIO (Memoria de Sesión)
                st.session_state['historial_predicciones'].append(resultado)
    ...
    if resultados_actuales:
        df_rank = pd.DataFrame(resultados_actuales).sort_values("Convicción (%)", ascending=False)
        ...
        if ganador["Convicción (%)"] >= umbral_conviccion:
            ...
            st.info(f"👉 **Orden:** Compra {acc:.4f} acciones de {ganador['Activo']}.")

# --- SECCIÓN: EL DIARIO DEL CAPITÁN (HISTORIAL) ---
st.markdown("---")
st.subheader("📓 El Diario del Capitán (Historial de Sesión)")
if st.session_state['historial_predicciones']:
    df_historial = pd.DataFrame(st.session_state['historial_predicciones'])
    # Mostramos las últimas predicciones arriba
    st.table(df_historial.iloc[::-1].head(10))
    
    if st.button("Limpiar Memoria de Sesión"):
        st.session_state['historial_predicciones'] = []
        st.rerun()
else:
    st.caption("Aún no hay registros en la Caja Negra. Ejecuta el radar para empezar a grabar.")
